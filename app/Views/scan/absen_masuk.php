        ...
        <div class="form-container">
            ...
            <div class="tips-penggunaan">
                ...
                <ul>
                    ...
                    <li>Pastikan untuk berbicara dengan jelas agar sistem dapat memverifikasi absensi Anda.</li>
                </ul>
                <h4>Penggunaan:</h4>
                <ol>
                    ...
                    <li><strong>Absensi:</strong> Setelah QR Code dipindai, data akan muncul.</li>
                    <li><strong>Verifikasi Suara:</strong> Sistem akan meminta Anda untuk berbicara, sebagai konfirmasi absensi masuk.</li>
                </ol>
            </div>
            <div class="tombol">
                <a href="<?= base_url('/') ?>" class="btn btn-custom mr-2">Dashboard</a>
                <a href="<?= base_url('scan/absen_pulang') ?>" class="btn btn-primary">Absen Pulang</a>
            </div>
        </div>
    ...
    <script>
        ...
        async function activateCamera(deviceId) {
            try {
                ...
                scanQRCodeMasuk(); // Mulai pemindaian QR code
            } catch (error) {
        ...
        function scanQRCodeMasuk() {
            const videoElement = document.getElementById('video');
            const canvasElement = document.createElement('canvas');
            const canvasContext = canvasElement.getContext('2d');

            function detectQRCode() {
                if (videoElement.readyState === videoElement.HAVE_ENOUGH_DATA && isScanning) { // Cek jika pemindaian diizinkan
                    ...
                    if (qrCode && !isProcessing) { // Cek apakah QR Code terdeteksi dan pemrosesan belum berjalan
                        isProcessing = true; // Set flag menjadi true, tanda bahwa proses sedang berlangsung
                        console.log('QR Code detected:', qrCode.data);
                        handleQRCodeMasuk(qrCode.data); // Proses data QR code

                        // Matikan kamera untuk 2 detik setelah QR Code dipindai
                        stopCameraTemporarily();
                    }
                }
                requestAnimationFrame(detectQRCode); // Lanjutkan pemindaian QR Code
            }

            detectQRCode(); // Mulai pemindaian
        }
        ...
        async function handleQRCodeMasuk(qrData) {
            ...
            const response = await fetch('<?= base_url('scan/absen_masuk/processQR') ?>', {
                method: 'POST',
                headers: {
            ...
            if ((result.status === 'error') &&
                (result.message === 'Anda sudah absen masuk.' || result.message === 'Absensi hari ini sudah terdaftar, istirahat yang cukup ya! Sampai besok.')) {
                ...
                if (result.message === 'Anda sudah absen masuk.') {
                    document.getElementById('jamPulang').textContent = result.absensi.jam_pulang ? result.absensi.jam_pulang : '-- : --';
                } else if (result.message === 'Absensi hari ini sudah terdaftar, istirahat yang cukup ya! Sampai besok.') {
                ...
                absensiResultElement.style.display = 'block'; // Menampilkan elemen absensi
                ...
                statusAbsensiElement.textContent = result.message;
                statusAbsensiElement.classList.remove('success');
                statusAbsensiElement.classList.add('errorr');
