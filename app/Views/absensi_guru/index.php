...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="<?= base_url('absensi_guru') ?>"><i class="fas fa-user-tie"></i> Absensi Guru</a>
            </li>
            <li class="nav-item">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="card-title">Daftar Absen Guru</h4>
                    <div class="d-flex align-items-center">
                        <form action="<?= base_url('absensi_guru') ?>" method="get" class="form-inline">
                            <!-- Input Tanggal -->
                            <input type="date" name="tanggal" id="tanggal" class="form-control custom-input mr-2"
                                value="<?= isset($tanggal) ? $tanggal : date('Y-m-d') ?>">

                            <!-- Input Keyword -->
                            <input type="text" name="keyword" class="form-control custom-input mr-2" placeholder="Nama/NUPTK"
                            ...
                            <tr>
                                <td colspan="8" class="text-center text-custom">
                                    <?php if ($tanggal && $keyword): ?>
                                        Tidak ada data absensi guru untuk pencarian ini.
                                    <?php elseif ($tanggal): ?>
                                        Tidak ada data absensi guru di tanggal ini.
                                    <?php elseif ($keyword): ?>
                                        Tidak ada data absensi guru untuk pencarian ini.
                                    <?php else: ?>
                                        Tidak ada data absensi guru ditemukan.
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php else: ?>
                            ...
                            $absensiGuruIds = array_map(function ($ag) {
                                return $ag['guru_id'];
                            }, $absensi_guru);
                            $allGuru = $guru;
                            ...
                            foreach ($allGuru as $g):
                                // Cek apakah guru ini sudah ada di absensi
                                $absenDitemukan = false;
                                foreach ($absensi_guru as $ag) {
                                    if ($ag['guru_id'] == $g['id']) {
                                        $absenDitemukan = true;
                                        break;
                                    }
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            foreach ($kehadiran as $k) {
                                                if ($k['id'] == $ag['kehadiran_id']) {
                                                    $statusKehadiran = $k['status'];
                                                    break;
                                                }
                                            }
                                            // Jika status kehadiran adalah "Tanpa Keterangan" dengan keterangan khusus
                                            if ($statusKehadiran == 'Tanpa Keterangan' && $ag['keterangan'] == 'Absensi jam pulang belum dilakukan') {
                                                echo "<span class='label label-tanpa-keterangan'>Tanpa Keterangan</span>";
                                            } else {
                                                ...
                                                echo "<span class='label $labelClass'>{$statusKehadiran}</span>";
                                            }
                                            ?>
                                        </td>
                                        <td><?= !empty($ag['jam_masuk']) ? date('H:i', strtotime($ag['jam_masuk'])) : '-- : --' ?></td>
                                        <td><?= !empty($ag['jam_pulang']) ? date('H:i', strtotime($ag['jam_pulang'])) : '-- : --' ?></td>
                                        <td><?= $ag['keterangan'] ?></td>
                                        <td>
                                            <a href="" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#editModal<?= $ag['id'] ?>">
                                ...
                                <?php else: ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            // Jika guru belum absen atau tanggal lebih besar dari hari ini
                                            if ($tanggal > $today): ?>
                                        ...
                                        <td>
                                            <!-- Tombol Tambah -->
                                            <a href="" class="btn btn-success btn-sm <?= $tanggal > $today ? 'btn-edit-disabled' : '' ?>"
                                                data-toggle="modal" data-target="#tambahModal<?= $g['id'] ?>">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>
                                        </td>
                                    </tr>
    ...
    <?php foreach ($guru as $g): ?>
        ...
        <div class="modal fade" id="tambahModal<?= $g['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="tambahModalLabel<?= $g['id'] ?>" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="<?= base_url('absensi_guru/tambah') ?>" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title" id="tambahModalLabel<?= $g['id'] ?>">Edit Absensi guru</h4>
                            <?php if (session()->getFlashdata('error') && session()->getFlashdata('modalError') === 'tambahModal' . $g['id']): ?>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="kehadiran<?= $g['id'] ?>">Kehadiran</label>
                                <div class="kehadiran-grid">
                                    <?php foreach ($kehadiran as $k): ?>
                                        <input type="radio" id="kehadiran<?= $g['id'] ?>_<?= $k['id'] ?>" name="kehadiran" value="<?= $k['id'] ?>"
                                            <?php if ($k['status'] == "Tanpa Keterangan"): ?> checked <?php endif; ?>>
                                        <label class="kehadiran-box <?= strtolower(str_replace(' ', '-', $k['status'])) ?>" for="kehadiran<?= $g['id'] ?>_<?= $k['id'] ?>">
                                            <?= $k['status'] ?>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <div class="form-group jam-container">
                                <div class="jam-item">
                                    <label for="jamMasuk<?= $g['id'] ?>">Jam Masuk</label>
                                    <input type="time" class="form-control" id="jamMasuk<?= $g['id'] ?>" name="jam_masuk">
                                </div>
                                <div class="jam-item">
                                    <label for="jamPulang<?= $g['id'] ?>">Jam Pulang</label>
                                    <input type="time" class="form-control" id="jamPulang<?= $g['id'] ?>" name="jam_pulang">
                                </div>
                            </div>

                            <!-- Input Keterangan -->
                            <div class="form-group">
                                <label for="keterangan<?= $g['id'] ?>">Keterangan</label>
                                <textarea class="form-control" id="keterangan<?= $g['id'] ?>" placeholder="Tulis keterangan di sini..." name="keterangan"></textarea>
                            </div>
                        </div>
    ...
    <?php foreach ($absensi_guru as $ag): ?>
        <!-- Modal Edit -->
        <div class="modal fade" id="editModal<?= $ag['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="editModalLabel<?= $ag['id'] ?>" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="<?= base_url('absensi_guru/edit/' . $ag['id']) ?>" method="post">
                        <div class="modal-header">
                            <h4 class="modal-title" id="editModalLabel<?= $ag['id'] ?>">Edit Absensi Guru</h4>
                            <?php if (session()->getFlashdata('error') && session()->getFlashdata('modalError') === 'editModal' . $ag['id']): ?>
                                <div class="alert alert-danger"><?= session()->getFlashdata('error') ?></div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Input Kehadiran -->
                            <div class="form-group">
                                <label for="kehadiran<?= $ag['id'] ?>" style="margin-top: 8px;">Kehadiran</label>
                                <div class="kehadiran-grid">
                                    <?php foreach ($kehadiran as $k): ?>
                                        <input type="radio" id="kehadiran<?= $ag['id'] ?>_<?= $k['id'] ?>" name="kehadiran" value="<?= $k['id'] ?>" <?= $ag['kehadiran_id'] == $k['id'] ? 'checked' : '' ?>>
                                        <label class="kehadiran-box <?= strtolower(str_replace(' ', '-', $k['status'])) ?>" for="kehadiran<?= $ag['id'] ?>_<?= $k['id'] ?>">
                                            <?= $k['status'] ?>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <div class="form-group jam-container">
                                <div class="jam-item">
                                    <label for="jamMasuk<?= $ag['id'] ?>">Jam Masuk</label>
                                    <input type="time" class="form-control" id="jamMasuk<?= $ag['id'] ?>" name="jam_masuk" value="<?= $ag['jam_masuk'] ?>">
                                </div>
                                <div class="jam-item">
                                    <label for="jamPulang<?= $ag['id'] ?>">Jam Pulang</label>
                                    <input type="time" class="form-control" id="jamPulang<?= $ag['id'] ?>" name="jam_pulang" value="<?= $ag['jam_pulang'] ?>">
                                </div>
                            </div>

                            <!-- Input Keterangan -->
                            <div class="form-group">
                                <label for="keterangan<?= $ag['id'] ?>">Keterangan</label>
                                <textarea class="form-control" id="keterangan<?= $ag['id'] ?>" placeholder="Tulis keterangan di sini..." name="keterangan"><?= $ag['keterangan'] ?></textarea>
                            </div>
                        </div>
